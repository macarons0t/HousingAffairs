---
title: "DSA5822 Project Group 5"
author: "Keane Looi A0135843A | Arisa Otsuka A0147095Y | Nick Keong A0156623E"
date: "10/25/2020"
output:
...
# We will now observe what the prices are for housing.
hist(dataset$price)
#Based on the histogram, we will split the groups of houses into prices between <30, 
...
price
# Based on the price leaflet, we observe a trend that houses further away from the city cluster 
...
# Based on the houseage leaflet, we observe that there is very weak correlation between the age 
...
# Based on the histogram, we observe that there is a high number of houses with 0 stores nearby, 
# and houses with 6-10 stores nearby are also few. Therefore, we decided to split the categories into 
# 0-1 stores, 2-5 stores and 6-10 stores. We are trying to keep the groups somewhat equal 
# in proportion.
...
# We observe that houses away from the city center generally 
...
# Therefore, we utilize the IQR provided by summary to split them into groups. 
...
# Based on the leaflet dist_mrt, we observe that houses away from the city center 
