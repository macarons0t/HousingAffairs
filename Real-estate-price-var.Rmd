...
##After doing a log x transformation, we obtained a smoother curve with an explainable trend. As crime rate increase, the median value of owner-occupied homes fall.
...
## Since this is a dummy variable, we cannot do a ggplot for this 
...
ggplot(dat, aes(x=tax, y=medv)) + geom_jitter(alpha = 0.4) + geom_smooth()
##KIV

#medv vs ptratio
ggplot(dat, aes(x=ptratio, y=medv)) + geom_jitter(alpha = 0.4) + geom_smooth()
##Again, we observe a downward trend where higher ptratio results in lower house prices. 
...
##There is a nice downward trend where the median price is lower at high levels of lstat (lower status of the population). 
##This is expected. 
```

## Plot against variables
##We did for all variables but only showing against rm as an example
```{r echo=TRUE}
#medv vs rm
ggplot(dat, aes(x=rm, y=medv)) + geom_point(position = "jitter", alpha = 0.4) + geom_smooth()
##There is a positive trend and that is expected because bigger houses 
...
lm1
##Most of the sign of the coefficients make sense except age where we expect it to be negative.
...
# Create residual plots for each variable, and study them closely
...
#RMSPE, AIC, R2 are all lower - hence we should use this model with the omitted influential points. 
